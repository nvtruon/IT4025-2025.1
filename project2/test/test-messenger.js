...
// Giải mã tin nhắn dùng khóa bí mật của chính phủ
const govDecrypt = async function (secret, [header, ct]) {
  // Giải mã khóa (message key) cho chính phủ rồi dùng để giải mã nội dung
  // Header PHẢI có trường "vGov"!!!
  let govKey = await computeDH(secret, header.vGov)
  govKey = await HMACtoAESKey(govKey, govEncryptionDataStr)

  // Header PHẢI có "cGov" và "ivGov"!!!
  // Dòng sau không đặt authenticatedData tùy chỉnh
  ...
  // Dùng authenticatedData là header để giải mã phần nội dung
